...
public:
    RedisConPool(size_t poolSize, const char* host, int port, const char* pwd)
        ...
        for (size_t i = 0; i < poolSize_; ++i) {
            ...
            if (context == nullptr || context->err != 0) {
                ...
                std::cout << "[Redis] Connection failed to " << host << ":" << port << std::endl;
                continue;
            }
        ...
        if (connections_.empty()) {
            std::cout << "[Redis] WARNING: No Redis connections available, will skip Redis cache" << std::endl;
        }
    ...
    redisContext* getConnection() {
        ...
        if (connections_.empty()) {
            std::cout << "[Redis] No available connections, skipping cache" << std::endl;
            return nullptr;
        }

        bool success = cond_.wait_for(lock, std::chrono::seconds(3), [this] {
            if (b_stop_) {
                return true;
            }
    ...
    void checkThread() {
        ...
        for (int i = 0; i < pool_size && !b_stop_; i++) {
            ...
            try {
                ...
                if (reply == nullptr || reply->type == REDIS_REPLY_ERROR) {
                    std::cout << "[Redis] Auth failed in check thread" << std::endl;
                    if (reply) freeReplyObject(reply);
                    continue;
                }
