...
void TcpMgr::initHandlers()
{
    ...
    // Echo测试回包
    _handlers.insert(ID_ECHO_RSP, [this](ReqId id, int len, QByteArray data) {
        Q_UNUSED(len);
        qDebug() << "handle id is " << id << " data is " << data;
        QJsonDocument jsonDoc = QJsonDocument::fromJson(data);
        if (jsonDoc.isNull()) {
        ...
        QJsonObject jsonObj = jsonDoc.object();
        if (!jsonObj.contains("error")) {
            qDebug() << "Echo Rsp error";
            return;
        }

        int err = jsonObj["error"].toInt();
        if (err != ErrorCodes::SUCCESS) {
            qDebug() << "Echo Failed, err is " << err;
            return;
        }

        QString data_str = jsonObj["data"].toString();
        qDebug() << "Echo Success, data: " << data_str;
        emit sig_echo_rsp(data_str);
      });

    // 获取用户信息回包
    _handlers.insert(ID_GET_USER_INFO_RSP, [this](ReqId id, int len, QByteArray data) {
        Q_UNUSED(len);
        qDebug() << "handle id is " << id << " data is " << data;
        QJsonDocument jsonDoc = QJsonDocument::fromJson(data);
        if (jsonDoc.isNull()) {
        ...
        QJsonObject jsonObj = jsonDoc.object();
        if (!jsonObj.contains("error")) {
            qDebug() << "GetUserInfo Rsp error";
            return;
        }

        int err = jsonObj["error"].toInt();
        if (err != ErrorCodes::SUCCESS) {
            qDebug() << "GetUserInfo Failed, err is " << err;
            return;
        }

        qDebug() << "GetUserInfo Success";
        emit sig_get_user_info_rsp(jsonObj);
      });
}
